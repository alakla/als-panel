 ...
 *
 * - Öffentliche Routen: Startseite, Login
 * - Admin-Bereich (/admin/*): Nur für Benutzer mit Rolle 'admin'
 * - Mitarbeiter-Bereich (/mitarbeiter/*): Nur für Benutzer mit Rolle 'mitarbeiter'
 * - Profil (/profile): Für alle angemeldeten Benutzer
 *
 * Sicherheit:
 * - 'auth': Nur angemeldete Benutzer haben Zugriff
 * - 'admin': Zusätzliche Prüfung: Benutzer muss Admin-Rolle haben
 * - 'mitarbeiter': Zusätzliche Prüfung: Benutzer muss Mitarbeiter-Rolle haben
 */
...
use App\Http\Controllers\Admin\FirmeneinstellungController;
...
/*
|--------------------------------------------------------------------------
| Dashboard-Weiterleitung (rollenabhängig)
|--------------------------------------------------------------------------
...
| Admin-Bereich (/admin/*)
|--------------------------------------------------------------------------
| Alle Routen in dieser Gruppe sind nur für Administratoren zugänglich.
| Middleware 'admin' prüft die Rolle des angemeldeten Benutzers.
*/
Route::middleware(['auth', 'admin'])->prefix('admin')->name('admin.')->group(function () {

    // Admin-Dashboard: Übersicht mit Kennzahlen
    ...
    // Monatsgehalt als bezahlt markieren
    Route::post('/mitarbeiter/{mitarbeiter}/bezahlen', [MitarbeiterController::class, 'bezahlen'])
        ->name('mitarbeiter.bezahlen');
    ...
    // Tätigkeitenverwaltung: Vordefinierte Beschreibungen für Zeiterfassung
    Route::resource('taetigkeiten', TaetigkeitController::class)
        ->only(['index', 'store', 'update', 'destroy'])
        ->parameters(['taetigkeiten' => 'taetigkeit']);

    // Firmeneinstellungen: Stammdaten die in Rechnungen erscheinen
    Route::get('/einstellungen', [FirmeneinstellungController::class, 'edit'])->name('einstellungen.edit');
    Route::put('/einstellungen', [FirmeneinstellungController::class, 'update'])->name('einstellungen.update');

    // Auftragsverwaltung: Admin sendet, verwaltet und gibt Aufträge frei
...
|--------------------------------------------------------------------------
| Alle Routen in dieser Gruppe sind nur für Mitarbeitende zugänglich.
*/
Route::middleware(['auth', 'mitarbeiter'])->prefix('mitarbeiter')->name('mitarbeiter.')->group(function () {

    // Mitarbeiter-Dashboard: Eigene Kennzahlen und letzte Einträge
    Route::get('/dashboard', [MitarbeiterDashboard::class, 'index'])->name('dashboard');

    // Aufträge: Mitarbeitende sehen ihre Aufträge und können sie bestätigen
...
|--------------------------------------------------------------------------
| Für alle angemeldeten Benutzer (Admin und Mitarbeiter).
*/
