 ...
 *
 * Dieser Controller ist verantwortlich fuer die Anzeige des Admin-Dashboards.
 * Er sammelt alle relevanten Kennzahlen (KPIs) und stellt sie der Ansicht bereit.
 * Zugriff nur fuer Benutzer mit der Rolle 'admin' (gesichert durch CheckAdmin-Middleware).
 */
class DashboardController extends Controller
{
     ...
     *
     * Folgende Daten werden an die Ansicht uebergeben:
     * - Anzahl aktiver Mitarbeitender
     * - Anzahl aktiver Auftraggeber
     * - Anzahl offener (nicht freigegebener) Zeiteintraege
    ...
    {
        // Anzahl der aktiven Mitarbeitenden zaehlen
        $mitarbeiterCount = Mitarbeiter::where('status', 'aktiv')->count();

        // Anzahl der aktiven Auftraggeber zaehlen
        $auftraggeberCount = Auftraggeber::where('is_active', true)->count();

        // Offene Zeiteintraege zaehlen (noch nicht freigegeben oder abgelehnt)
        ...
        // Daten an die Admin-Dashboard-Ansicht uebergeben
